...
def compute_tfidf(abstracts):
    ...
    tokenized_abstracts = [abstract['abstract'].lower().split() for abstract in abstracts]
...
def print_abstracts_and_scores():
    ...
    # Save up to 10 top abstracts to an external file
    if top_abstracts:
        with open("top_abstracts.txt", "w", encoding="utf-8") as file:
            for idx, article in enumerate(top_abstracts[:10]):
                ...
                file.write("-" * 80 + "\n")
        print("\nTop abstracts saved to 'top_abstracts.txt'.")
    else:
